...
def format_unified_prompt(entry):
    option_lines = "\n".join(f"{opt} ; Lexical Unit definition - {lu_def}" for opt, lu_def in zip(entry["frame_definitions"], entry['lu_definitions']))
...
- Target Word
- A list of frames with their definitions and associated lexical unit descriptions.
...
Frames:
Foreign_or_domestic_country - A Current_country is identified as matching or not matching the instance of the Discourse_salient_country.  'Foreign sales account for only 20% of IBM's income .'; Lexical Unit definition - international.a : of multiple nations, especially excluding the current one.,
Political_locales - This frame covers words that name Locations as defined politically, or administratively.  'Berkeley is a wonderful city.' ; Lexical Unit definition - international.a : pertaining to multiple nations.
